...
const MANUAL_JERARQUIAS = [
  {
    tipo: 'parrafo',
    texto:
      'La jerarquía es la categoría organizacional de cada usuario (por ejemplo Comensal, Cocina, ' +
      'Gerencia, Admin). Determina qué pantallas puede ver y qué acciones puede hacer en el sistema.',
  },
  { tipo: 'subtitulo', texto: 'Campos' },
  {
    ...
    body: [
      ...
      ['Bonificación (%)', 'Ver aviso más abajo'],
    ],
  },
  {
    tipo: 'parrafo',
    texto:
      'Aviso: el campo "Bonificación (%)" de esta pantalla queda guardado, pero hoy NO determina el ' +
      'descuento real que paga el comensal al pedir. El descuento efectivo lo decide el motor de Reglas ' +
      'de Bonificación (Configuración → Reglas de Bonificación), que sí puede usar la Jerarquía como una ' +
      'de sus condiciones (por ejemplo, "esta regla aplica solo a la jerarquía Comensal").',
  },
  { tipo: 'subtitulo', texto: 'Particularidades' },
  {
    tipo: 'lista',
    items: [
      'Las jerarquías Admin, Cocina, Comensal y Gerencia son las que usa el sistema de acceso: no se pueden renombrar ni eliminar.',
      'La jerarquía "predeterminada" se marca desde el listado (botón con forma de estrella), no desde el formulario de alta/edición.',
    ],
  },
];
...
const Jerarquia = () => {
  ...
  const cargarJerarquias = useCallback(async (page = 1, searchTerm = '', mostrarActivos = true) => {
    try {
      ...
      const data = await jerarquiasService.getJerarquiasLista(page, pageSize, searchTerm, mostrarActivos);
      ...
      const totalItemsBackend = data.totalItems ?? data.totalCount ?? jerarquiasNormalizadas.length;
  ...
  return (
    <div className="container-fluid" style={{ padding: 0 }}>
      ...
      <div className="page-title-bar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <BotonAyuda titulo="Manual — Jerarquías" contenido={MANUAL_JERARQUIAS} nombreArchivo="manual_jerarquias.pdf" />
      </div>
          ...
          <div className="d-flex justify-content-between align-items-center mt-3 mb-4 flex-nowrap" style={{ gap: '1.5rem' }}>
            <div className="d-flex align-items-center flex-nowrap" style={{ gap: '1.25rem' }}>
              ...
              <span className="text-muted" style={{ whiteSpace: 'nowrap', fontSize: '0.9rem' }}>
                Mostrando página {currentPage} de {totalPages} ({totalItems} jerarquías)
              </span>
            </div>
            <nav>
